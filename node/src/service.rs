...
// Local Runtime Types
use Pixel_runtime::{opaque::Block, Hash, RuntimeApi};
...
type FullClient = TFullClient<Block, RuntimeApi, NativeElseWasmExecutor<TemplateRuntimeExecutor>>;
    ...
    ) -> Result<Box<dyn ParachainConsensus<Block>>, sc_service::Error>,
{
    ...
    let sc_service::PartialComponents {
        ...
        transaction_pool,
        other:
            (filter_pool, frontier_backend, mut telemetry, telemetry_worker_handle, fee_history_cache),
    } = new_partial(&mut parachain_config, false)?;
...
pub fn start_instant_seal_node(config: Configuration) -> Result<TaskManager, sc_service::Error> {
    let mut config = prepare_node_config(config);
    let sc_service::PartialComponents {
        client,
        backend,
        mut task_manager,
        ...
        transaction_pool,
        other:
            (filter_pool, frontier_backend, mut telemetry, _telemetry_worker_handle, fee_history_cache),
